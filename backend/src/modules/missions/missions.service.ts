...
export class MissionsService {
  async getAvailableMissions(carrierId: string, query: AvailableMissionsQuery) {
    ...
    const pendingParcels = await prisma.parcel.findMany({
      where: {
        ...
        pickupSlotEnd: { gte: new Date() }, // Vérifier que le créneau n'est pas expiré
  ...
  async acceptMission(carrierId: string, parcelId: string) {
    ...
    if (parcel.vendor.fcmToken && carrier) {
      const { notificationService } = await import('../../shared/services/notification.service.js');
      const carrierName = `${carrier.firstName} ${carrier.lastName}`;
      await notificationService.send(
        parcel.vendor.fcmToken,
        'Mission acceptée !',
        `${carrierName} prendra en charge votre colis.`,
        {
  ...
  async getCurrentMissions(carrierId: string) {
    const missions = await prisma.mission.findMany({
      where: {
        carrierId,
        status: { in: [MissionStatus.ACCEPTED, MissionStatus.IN_PROGRESS, MissionStatus.PICKED_UP] },
      },
      include: {
        parcel: {
          include: {
