...
export class PaymentsController {
  ...
  async getTransactionsByRole(
    request: FastifyRequest<{ Params: { role: 'payer' | 'payee' } }>,
    reply: FastifyReply
  ) {
    try {
      const userId = (request.user as any).userId;
      ...
      if (role !== 'payer' && role !== 'payee') {
        return reply.status(400).send({ error: 'Rôle invalide. Utilisez "payer" ou "payee".' });
      }

      const result = await this.paymentsService.getTransactionsByRole(
        userId,
        role,
        parseInt(page),
        parseInt(limit)
  ...
  async getCarrierEarnings(request: FastifyRequest, reply: FastifyReply) {
    try {
      const userId = (request.user as any).userId;
      const result = await this.paymentsService.getCarrierEarnings(userId);
      return reply.send(result);
    } catch (error: any) {
      return reply.status(500).send({ error: error.message });
    }
