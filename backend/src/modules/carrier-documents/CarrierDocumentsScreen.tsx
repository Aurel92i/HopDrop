...
// ✅ Ajout de DRIVING_LICENSE dans les labels
const documentLabels: Record<DocumentType, { label: string; description: string; icon: string }> = {
  ...
  DRIVING_LICENSE: {
    ...
    description: 'Obligatoire pour scooter et voiture',
    icon: 'card-account-details',
  },
};

const vehicleOptions = [
  { value: 'NONE', label: 'Aucun', icon: 'walk' },
...
export function CarrierDocumentsScreen() {
  ...
  const handleVehicleChange = async (value: string) => {
    try {
      ...
      // Recharger les documents car les requirements peuvent changer
      loadDocuments();
    } catch (error: any) {
      Alert.alert('Erreur', error.message);
    }
  ...
  // ✅ Helper pour savoir quels documents sont requis selon le véhicule
  const getRequiredDocumentsInfo = (vehicleType: VehicleType) => {
    switch (vehicleType) {
      case 'CAR':
        return 'Carte d\'identité + Permis de conduire + Carte grise';
      case 'SCOOTER':
        return 'Carte d\'identité + Permis de conduire';
      default:
        return 'Carte d\'identité uniquement';
    }
  };
  ...
  const pendingCount = documents.filter((d) => d.required && d.status === 'PENDING').length;
  const missingCount = documents.filter((d) => d.required && !d.status).length;
    ...
    >
          ...
          <View style={styles.statusInfo}>
            ...
            <Text variant="bodySmall" style={styles.statusSubtext}>
              {allRequiredApproved
                ...
                : missingCount > 0
                ? `${missingCount} document(s) manquant(s)`
                : pendingCount > 0
                ? `${pendingCount} document(s) en attente de validation`
      ...
      <Card style={styles.card}>
        <Card.Content>
            ...
            {vehicleOptions.map((option) => (
              ...
              >
                <MaterialCommunityIcons
                  ...
                  size={20}
                  color={profile?.vehicleType === option.value ? colors.primary : colors.onSurfaceVariant}
                />
                <Text
                  style={[
          ...
          {/* ✅ Info sur les documents requis selon le véhicule */}
          {profile?.vehicleType && (
            <View style={styles.vehicleInfoBox}>
              <MaterialCommunityIcons name="information" size={16} color={colors.primary} />
              <Text variant="bodySmall" style={styles.vehicleInfoText}>
                Documents requis : {getRequiredDocumentsInfo(profile.vehicleType)}
              </Text>
            </View>
          )}
        </Card.Content>
        ...
        <Card style={styles.emptyCard}>
          <Card.Content>
            <Text style={styles.emptyText}>
              Sélectionnez d'abord votre type de véhicule pour voir les documents requis.
            </Text>
          </Card.Content>
        </Card>
      ) : (
              ...
              <Card.Content>
                <View style={styles.documentHeader}>
                  <MaterialCommunityIcons
                    ...
                    size={28}
                    color={doc.status === 'APPROVED' ? colors.primary : doc.status === 'REJECTED' ? colors.error : colors.outline}
                  />
                ...
                <View style={styles.documentActions}>
                  {!doc.uploaded || doc.status === 'REJECTED' ? (
                    ...
                    >
                      {doc.status === 'REJECTED' ? 'Renvoyer' : 'Ajouter'}
                    </Button>
                  ) : (
      ...
      <View style={styles.infoBox}>
        ...
        <Text variant="bodySmall" style={styles.infoText}>
          Vos documents seront vérifiés sous 24 à 48 heures. Vous recevrez une notification une fois
